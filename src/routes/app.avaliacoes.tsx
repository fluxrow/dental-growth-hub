  ...
  return (
    <AppShell title="Avaliações" subtitle="Reputação no Google · pedidos automáticos pós-atendimento">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {REVIEW_KPIS.map((k) => <KpiCard key={k.key} k={k}/>)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <section className="lg:col-span-2 rounded-xl border border-border bg-surface overflow-hidden">
          <div className="px-4 h-12 border-b border-border flex items-center justify-between">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">Atendidos nos últimos 7 dias sem pedido enviado</div>
            </div>
          </div>
          <ul className="divide-y divide-border">
            {ELIGIBLE_FOR_REVIEW.map((p) => (
              <li key={p.id} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11.5px] text-muted-foreground">Última visita: {p.lastVisit}</div>
                </div>
            ...
            {ELIGIBLE_FOR_REVIEW.length === 0 && (
              <li className="px-4 py-6 text-center text-[12.5px] text-muted-foreground">Nenhum paciente elegível no momento.</li>
            )}
          </ul>
        </section>
        ...
        <section className="lg:col-span-3 rounded-xl border border-border bg-surface overflow-hidden">
          <div className="px-4 h-12 border-b border-border flex items-center justify-between">
            <div>
              <div className="text-[13px] font-semibold tracking-tight">Avaliações recentes no Google</div>
              <div className="text-[11px] text-muted-foreground">Importadas automaticamente · análise de sentimento por IA</div>
            </div>
            <div className="inline-flex items-center gap-1 text-warning">
              ...
              <span className="ml-1.5 text-[12px] font-semibold text-foreground tabular-nums">4.8</span>
            </div>
          </div>
          <ul className="divide-y divide-border">
            {REVIEWS.map((r) => (
              <li key={r.id} className="px-4 py-4">
                <div className="flex items-baseline justify-between gap-3 mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn(
                      ...
                      r.sentiment === "neutral" ? "bg-warning/15 text-warning-foreground" :
                      "bg-destructive/10 text-destructive",
                    )}>
                      {r.sentiment === "positive" ? "positiva" : r.sentiment === "neutral" ? "neutra" : "negativa"}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-0.5 text-warning mb-1.5">
                  ...
                  {Array.from({ length: 5 - r.rating }).map((_, i) => <Star key={i} className="size-3 text-muted"/>)}
                </div>
