    ...
    <AppShell title="Conversas" subtitle="Inbox unificado · WhatsApp + canais conectados" flush>
      <div className="grid grid-cols-12 h-[calc(100vh-4rem)]">
        ...
        <aside className="col-span-12 md:col-span-4 lg:col-span-3 border-r border-border bg-surface flex flex-col min-w-0">
          <div className="p-3 border-b border-border">
            <div className="relative">
              ...
              <input placeholder="Buscar conversa…" className="h-8 w-full rounded-md border border-input bg-background pl-8 pr-3 text-[12px] focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div className="flex gap-1 mt-2 text-[11px]">
              {["Todas","Não lidas","Aguardando"].map((f, i) => (
                <button key={f} className={cn("px-2 py-0.5 rounded-md", i === 0 ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>{f}</button>
              ))}
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="text-[10.5px] text-muted-foreground shrink-0 tabular-nums">{c.lastTime}</span>
                    </div>
                    <div className="text-[12px] text-muted-foreground truncate">{c.lastMessage}</div>
                    <div className="mt-1 flex items-center gap-1.5">
                      <span className={cn("inline-flex rounded-full px-1.5 py-0.5 text-[9.5px] font-medium", s.tone)}>{s.label}</span>
                      {c.unread > 0 && (
                        <span className="ml-auto size-4 rounded-full bg-primary text-primary-foreground text-[9.5px] font-semibold flex items-center justify-center tabular-nums">{c.unread}</span>
                      )}
                    </div>
                  </div>
                </button>
        ...
        <section className="hidden md:flex col-span-8 lg:col-span-6 flex-col bg-background min-w-0">
          <div className="h-14 border-b border-border flex items-center justify-between px-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="size-9 rounded-full bg-gradient-to-br from-muted to-accent flex items-center justify-center text-[11px] font-semibold">
                {active.patientName.split(" ").map(n => n[0]).slice(0,2).join("")}
              </div>
              <div className="min-w-0">
                <div className="text-[13.5px] font-semibold truncate">{active.patientName}</div>
                <div className="text-[11.5px] text-muted-foreground truncate">{active.phone} · {active.source}</div>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <button className="size-8 rounded-md hover:bg-muted flex items-center justify-center"><Phone className="size-4 text-muted-foreground"/></button>
              <button className="size-8 rounded-md hover:bg-muted flex items-center justify-center"><MoreVertical className="size-4 text-muted-foreground"/></button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-3">
            ...
            {active.messages.map((m) => (
              <div key={m.id} className={cn("flex", m.from === "patient" ? "justify-start" : "justify-end")}>
                <div className={cn(
                  ...
                  {m.text}
                  <div className={cn("text-[10px] mt-0.5 text-right tabular-nums", m.from === "patient" ? "text-muted-foreground" : "text-primary-foreground/70")}>{m.time}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-border bg-surface p-3">
            <div className="flex items-center gap-1 mb-2">
              {["Olá! 😊","Confirmar consulta","Enviar orçamento","Reagendar"].map((t) => (
                <button key={t} className="text-[11px] px-2 py-1 rounded-md border border-input bg-background hover:bg-muted">{t}</button>
              ))}
            </div>
            <div className="flex items-end gap-2">
              <button className="size-9 rounded-md hover:bg-muted flex items-center justify-center"><Paperclip className="size-4 text-muted-foreground"/></button>
              <div className="flex-1 rounded-lg border border-input bg-background px-3 py-2">
                <textarea rows={1} placeholder="Escreva uma mensagem…" className="w-full resize-none bg-transparent text-[13.5px] focus:outline-none" />
              </div>
              <button className="size-9 rounded-md hover:bg-muted flex items-center justify-center"><Smile className="size-4 text-muted-foreground"/></button>
        ...
        <aside className="hidden lg:flex col-span-3 flex-col border-l border-border bg-surface min-w-0">
          <div className="h-14 border-b border-border flex items-center justify-between px-4">
            <span className="text-[12px] font-semibold uppercase tracking-wider text-muted-foreground">Contexto do paciente</span>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-5">
            ...
            <div>
              ...
              <div className="text-[13px] font-medium tabular-nums">R$ {active.estValue.toLocaleString("pt-BR")}</div>
            </div>
            <div>
              <div className="text-[11px] text-muted-foreground mb-1.5">Tags</div>
              <div className="flex flex-wrap gap-1">
                {active.tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 rounded-md bg-accent text-accent-foreground px-1.5 py-0.5 text-[10.5px]">
