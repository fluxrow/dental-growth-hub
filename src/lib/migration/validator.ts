  ...
  rows.forEach((row, idx) => {
    ...
    if (!rawName || rawName.length < 2) {
      errorRows.push({
        ...
        field: "name",
        value: rawName,
        reason: "Nome obrigatório (mín. 2 caracteres)",
      });
      hasError = true;
    } else {
    ...
    if (!normalizedPhone) {
      errorRows.push({
        ...
        field: "phone",
        value: rawPhone,
        reason: "Telefone inválido após normalização",
      });
      hasError = true;
    ...
    if (fieldToCol["email"]) {
      const rawEmail = String(row[fieldToCol["email"]] ?? "")
        .trim()
        .toLowerCase();
      if (rawEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(rawEmail)) {
        warningRows.push({
          ...
          field: "email",
          value: rawEmail,
          suggestion: "Email com formato inválido — será ignorado",
        });
        hasWarning = true;
      } else {
      ...
      if (rawDate) {
        ...
        if (!d || isNaN(d.getTime())) {
          warningRows.push({
            ...
            field: "last_visit_at",
            value: rawDate,
            suggestion: "Formato de data não reconhecido — será ignorado",
          });
          hasWarning = true;
        } else {
      ...
      if (rawLtv) {
        ...
        if (n === null) {
          warningRows.push({
            ...
            field: "ltv",
            value: rawLtv,
            suggestion: "Valor não reconhecido — será importado como 0",
          });
          hasWarning = true;
          norm.ltv = 0;
        } else {
