...
function phone(i: number) {
  return `+55 11 9${(10000 + i * 173).toString().slice(0, 4)}-${(1000 + i * 31).toString().slice(0, 4)}`;
}
...
export async function seedDemoData(
  ...
  const atividades: Array<{ kind: Kind; title: string; detail?: string; value?: number }> = [
    {
      kind: "resposta",
      title: `${insertedPatients![0].name} respondeu no WhatsApp`,
      detail: "Confirmou interesse no orçamento",
    },
    {
      kind: "confirmacao",
      title: `${insertedPatients![1].name} confirmou consulta`,
      detail: "Avaliação amanhã 14h",
    },
    {
      kind: "pagamento_confirmado",
      title: `Pagamento confirmado · ${insertedPatients![2].name}`,
      value: 450,
    },
    {
      kind: "cobranca_enviada",
      title: `Cobrança enviada · ${insertedPatients![3].name}`,
      value: 1200,
    },
    { kind: "avaliacao", title: `${insertedPatients![4].name} avaliou 5 estrelas no Google` },
    {
      kind: "falha",
      title: `Falha no envio para ${insertedPatients![5].name}`,
      detail: "WhatsApp não entregue",
    },
    {
      kind: "pagamento_atrasado",
      title: `${insertedPatients![6].name} com cobrança vencida`,
      value: 800,
    },
    {
      kind: "cobranca_respondida",
      title: `${insertedPatients![7].name} respondeu sobre pagamento`,
    },
  ...
  const notificacoes = atividadesRows.slice(0, 6).map((a) => {
    const fin =
      a.kind === "pagamento_confirmado" ||
      a.kind === "cobranca_enviada" ||
      a.kind === "pagamento_atrasado" ||
      a.kind === "cobranca_falhou";
    return {
      clinic_id: clinicId,
      user_id: userId,
